...
(function () {
    ("use strict");
var paymentSign = "$";
Array.from(document.getElementsByClassName("product-line-price")).forEach(function (key,item) {
	item.value = paymentSign + document.getElementById(key.id).value
});
...
var count = (Number(document.getElementById("counter_value").value));
...
function new_link() {
	...
	var appendDiv = "<select class='form-selectborder-0 bg-light' data-choices data-choices-search-false onchange='getProductDescription(this.value , this.id)' id='productName-" + count + "'  required name='product[]'><option value=''>Select Product</option>";
	...
	var delLink =
		...
		"</td>" +
		"<td>" +
		'<input class="form-control bg-light border-0 product-price" type="number" name="rate[]" id="productRate-' + count + '" step="0.01" placeholder="$0.00">' +
		"</td>" +
		"<td>" +
		'<div class="input-step">' +
		'<button type="button" class="minus">–</button>' +
		'<input type="number" class="product-quantity" name="quantity[]" id="product-qty-' + count + '" value="0" readonly>' +
		'<button type="button" class="plus">+</button>' +
		"</div>" +
		"</td>" +
		'<td class="text-end">' +
		"<div>" +
		'<input type="text" class="form-control bg-light border-0 product-line-price" name="amount[]" id="productPrice-' + count + '" value="$0.00" placeholder="$0.00" />' +
		"</div>" +
		"</td>" +
...
if ((viewobj != '') && (options == "edit-invoice")) {
	...
	var counter_1 = (Number(document.getElementById("counter_value").value));
